
<p align="center">
  <img src="https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/c75b8958eab47dd88b4f3dd049431e3615b24f2d/img/photo-pour-github.jpg" width="160" style="border-radius: 100%;"/>
</p>

<h1 align="center">Élodie MENDES</h1>
<h3 align="center">Data Analyst • Cheffe de Projet Data</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/elodie-mendes-9905ba116/">
    <img src="https://img.shields.io/badge/LinkedIn-Elodie%20Mendes-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>
---

## À propos

10 ans d’expérience en **achats internationaux**, aujourd’hui spécialisée dans :

- Analyse & visualisation de données  
- Machine Learning (supervisé / non supervisé)  
- Gestion de projet data  
- SQL, modélisation & qualité des données  
- Tableaux de bord interactifs (Tableau / Power BI)

---

## Projets Data

### Analyse de vente pour la grande distribution
![marketing](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/Le%20grand%20March%C3%A9%20banni%C3%A8re.png)

**Data visualisation, Storytelling**

*Contexte*: Une entreprise de grande distribution en ligne a besoin du rapport mensuel des actions marketing.

*Objectifs*: Analyser les résultats des actions marketing menés par le service marketing et présenter les résultats à l'aide du storytelling.

*Actions*: Conception de graphiques sur Excel, organisation de la présentation selon les principes du storytelling.

*Résultats*: Proposition des recommandations des actions à mettre en place à destinantion de l'équipe marketing. 

➡️ [Voir le projet](https://github.com/Elodie-M7/Portfolio-Data-Analyst/tree/435efbf008e94f08ae1361facf9b15bee76f0dcd/Mes_projets/Analyse%20marketing%20Grande%20Distribution)

### 📘 Analyse des ventes d’une librairie
![librairie](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/06112aae6dadc525fd47262a6fdeb69ee76d7eea/img/librairie%20en%20ligne.png)

**Python, Pandas,Tests statistiques, corrélations** 

*Contexte*: Une librairie physique se décide après plusieurs années de se lancer dans la vente en ligne.

*Objectifs*: Mieux comprendre le comportement d'achat de ses clients en ligne.

*Actions*: Application de tests statistiques pour chercher des corrélations.

*Résultats*:  Mise en évidence de corrélations (âge/panier moyen ou genre/catégorie de livre acheté). Recommandations marketing et commerciales pour augmenter les ventes.

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/130af05039fa599187305c7b8a316fe803de7a85/Mes_projets/Analyse%20des%20ventes%20d%E2%80%99une%20librairie%20en%20ligne)

---

### 📦 Optimisation des données e-commerce  
![boutique vin](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/boutique%20vin.png)

**Python,Consolidation, nettoyage, EDA**  

*Contexte*: Un boutique de vins et spiritieux en ligne a des difficultés à gérer ses stocks et analyser les performances de son activité.

*Objectifs*: Assurer l'interopérabilité des données entre l'ERP interne à l'entreprise et ceux de son site e-commerce.

*Actions*: Collecte, nettoyage de données, consolisation et harmonisation des données, analyse des données de ventes et de stock.

*Résultats*: centralisation des flux de données entre l'ERP/site e-commerce, automatisation du contrôle de la qualité des données, proposition de création de tableau de bord intéractif pour mieux piloter l'activité.

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Optimiser%20la%20gestion%20des%20donn%C3%A9es%20boutique%20e-commerce)

---

### 🩺 Étude de santé publique
![santé FAO](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/etude%20sante%20publique.png)

**Python,Nettoyage, analyses exploratoires**

*Contexte*: la FAO est un des organes qui composent l’ONU dont l’objectif est d'éradiquer la faim dans le monde.

*Objectifs*: Réaliser des analyses spécifiques sur la sous-nutrition entre 2013 et 2017.

*Actions*: Collecte de données, préparation et traitement de données, analyse exploratoire de données.

*Résultats*: Insights clés, interprétation des résultats et plans d'actions prioritaires pour l'organisme.

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Etude%20sur%20la%20faim%20dans%20le%20monde)

---

### 🏠 Base de données immobilière SQL
![immobilier](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/agence%20immobili%C3%A8re.png)

**Modélisation (MCD), +20 requêtes SQL**

*Contexte*: Une agence immobilière a lancé le projet "DATAImmo" pour créer un modèle prédictif capable d’estimer le prix de vente des biens immobiliers. 

*Objectifs*: Modifier et exploiter la base de données des transactions immobilières afin d’analyser le marché et d’aider les agences régionales.

*Actions*: Collecte de données, Normalisation (3NF) et Modélisation des données, requêter et manipuler des bases de données relationnelles.

*Résultats*: Base de données normalisées et prête pour répondre aux requêtes de l'agence sur son marché , interprétation des résultats.

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Cr%C3%A9er%20et%20G%C3%A9rer%20une%20base%20de%20donn%C3%A9es%20Immobili%C3%A8re)

---

### 💶 Détection de faux billets 
![finance](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/detecter%20faux%20billet.png)

**Scikit-learn,ACP, régression logistique, modèles supervisés**

*Contexte* : L’ONCFM souhaite améliorer la détection des faux billets.  

*Objectif* : Développer une application pour automatiser ce processus.  

*Actions* : Préparer et analyser les données, entraîner différents modèles prédictifs (supervisés et non supervisés), puis comparer leurs performances. 

*Résultat* : Mise en place d’une application performante pour la détection automatique des faux billets.

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/D%C3%A9tectez%20des%20faux%20billets)

---

### 📊 Avancement de projets (Power BI)  

**Dashboard interactif**  Suivi coûts • délais • qualité  

*Contexte* : Une entreprise dans les soins bucco-dentaires a plusieurs projets en cours et n'arrive plus à suivre son activité.

*Objectif* :  Créer un tableau de bord pour piloter ses projets et suivre leurs performances pour les 3 directions (Général, Régional et Pays)

*Actions* :  Mise en place du Product Strategy Canvas (Formaliser les besoins pour chaque directeur), création de graphiques, construction d'indicateurs clés de performance

*Résultat* : Tableau de bord intéractif avec les projets en alerte pour prioriser les actions. 

➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Visualiser%20l'avancement%20de%20projets)

---

### 💧 Étude sur l’eau potable
![eau potable](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/etude%20eau%20potable.png)

**Collecte, préparation et analyse,Power Query** 

*Contexte* : L'ONG DWFA (Drinking Water For All) a pour ambition de donner accès à l’eau potable à tout le monde en mettant agisant sur 3 domaines d'expertise: 
Création de services d’accès à l’eau potable, modernisation de services d’accès à l’eau déjà existants et consulting auprès d’administrations/gouvernements à propos des politiques d’accès à l’eau

*Objectif* : Créer un tableau de bord pour identifier les pays qui rencontrent des difficultés d’accès à l’eau potable et identifier ceux pour lesquels concentrer les efforts. 

*Actions* :  Mock-up/Product Strategy Canvas, prétraitement et manipulation des données.

*Résultat* : Tableau interactif mettant en valeur les pays qui sont en fort stress hydrique.


➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Etude%20sur%20l%E2%80%99eau%20potable)

---

### 🐔 Étude de marché (aviculteur)  
![aviculteur](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/14e870c12386b454fcffd79204738e48f455914d/img/etude%20de%20marche%20aviculteur.png)

**Machine Learning non supervisé : ACP + clustering**

*Contexte* : Un aviculteur souhaitait exporter ses produits à l’international mais ne savait pas quels pays cibler.

*Objectif* : Identifier des groupes de pays présentant un fort potentiel pour le projet d’exportation. 

*Actions* : Collecte et préparation des données, application de méthodes d’apprentissage non supervisé (clustering et analyse en composantes principales), puis création d’un tableau de score pour classer les pays les plus prometteurs.  

*Résultat* : Sélection de trois pays prioritaires pour débuter les exportations.


➡️ [Voir le projet](https://github.com/Elodie-M7/Portofolio-Data-Analyst/tree/fca23d148147b3996522034e79568c7118dd62dd/Mes_projets/Etude%20de%20march%C3%A9%20pour%20projet%20d'export)

### ✈️ Livrables AéroWorld  

Gestion de projet, veille métier & dataviz, vidéo de formation, documentation.  
➡️ [Voir](https://github.com/Elodie-M7/Dossier-de-candidature-A-roworld/tree/bf15a8f4dc7c32eb91e27c5e03de7333a67fce4f/Livrables)

*Contexte*: Une entreprise dans l'aéronautique recherche un chef de projet Data pour l'accompagner dans sa transformation digitale

*Objectif* : Créer un portfolio pour postuler au poste de chef de projet Data

*Actions* :  gestion du projet (Cachier des charges, Diagramme de Gantt), Analyse des besoins métiers, production de contenus pédagogiques

*Résultat* : Portfolio contenant les livrables demandés pour le dossier de candidature (Tableau de bord de profil, Tableau de bord de veille métier, Vidéo de formation, Documentation)

---

## 🛠️ Compétences

**Data**  
Python • Pandas • SQL • Tableau • Power BI • Machine Learning • Statistiques  

**Gestion & Business**  
Achats internationaux • KPI • Sourcing Asie • Pricing • Logistique • Conformité  

---

## 💡 Soft Skills

Analytique • Curiosité • Storytelling • Adaptation • Empathie • Rigueur

---

## 🎓 Formations

- Data Analyst — OpenClassrooms  
- Master Management de Projet — Ascencia Business School  
- Licence AES — Sorbonne Paris Nord  
- DUT Techniques de Commercialisation — Sorbonne Paris Nord  

---

## 📬 Contact
![linkedin](https://github.com/Elodie-M7/Portfolio-Data-Analyst/blob/fc6fb080a872c61875d14ab9bde0179a53d5561d/img/Profil%20Linkedin.png)

📎 **LinkedIn** :  
[www.linkedin.com/in/elodie-mendes-9905ba116](https://www.linkedin.com/in/elodie-mendes-9905ba116/)
